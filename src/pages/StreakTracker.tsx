...
function StreakTracker () {
    ...
    const UserActivityData = useContext(contexts.UserActivityContext).userActivities;
    ...
    // const calendarInfo = [date, month, year];

    return(
        <div id="streak-tracker-container">
            <p id = "streak-tracker-heading">🔥 Streak Tracker</p>
            <div>Current Streak : <span>{curr_streak} Day{(curr_streak > 1) ? "s" : ""}</span></div>
            <div>Max Streak : <span>{max_streak} Day{(max_streak>1) ? "s" : ""}</span></div>
            <div>Up for Today : <span>{todaysPending} {(todaysPending>1) ? "Activities" : "Activity"}</span></div>

            {/* //TODO: make render calendar component */}
            {/* <div>
